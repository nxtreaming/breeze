...
func runGenerate(args []string) error {
        if len(args) < 2 {
                return fmt.Errorf("usage: breeze generate <handler|resource|grpc> <Name> [args...]")
        }
        ...
        case "grpc":
                return generateGRPC(modulePath, name, rest)
        default:
                return fmt.Errorf("unknown generator %q — must be handler, resource, or grpc", kind)
        }
}

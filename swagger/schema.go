...
func walkType(t reflect.Type) *Schema {
	...
	case reflect.Map:
		// Represent as object with no fixed properties
...
func structSchema(t reflect.Type) *Schema {
	...
	for i := 0; i < t.NumField(); i++ {
		...
		// Resolve field name from json tag
		...
		// Pick up a description tag if present
		if desc := f.Tag.Get("description"); desc != "" {
			schema.Description = desc
		}
		// Pick up example tag if present
		...
		// Track required fields (exported, no omitempty)
		if !omitempty {
